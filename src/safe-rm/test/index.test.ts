...
describe("safe rm", () => {
  ...
  test("requires recursive intent and protects Git metadata", async () => {
    ...
    await expect(
      setup().execute(
        "call-6",
        { paths: [join(externalMetadata, "config")] },
        undefined,
        undefined,
        { cwd },
      ),
  ...
  test("rejects paths that escape through a parent symlink", async () => {
    ...
    await expect(
      setup().execute("call-7", { paths: ["outside/hosts"] }, undefined, undefined, { cwd }),
  ...
  test("refuses direct, nested, and symlink-aliased credentials", async () => {
    ...
    for (const params of [
      ...
      { paths: ["@.env"] },
      { paths: ["ordinary.txt"] },
      { paths: ["output"], recursive: true },
    ]) {
      await expect(
        setup().execute("credential", params, undefined, undefined, { cwd }),
  ...
  test("refuses a recursive parent containing nested Git metadata", async () => {
    ...
    await expect(
      setup().execute(
        "nested-git",
        { paths: ["artifacts"], recursive: true },
        undefined,
        undefined,
        { cwd },
      ),
    ...
    expect(
      await Bun.file(join(cwd, "artifacts", "checkout", ".git", "config")).exists(),
    ).toBeTrue();
  });
  ...
  test("honors cancellation before removing anything", async () => {
    ...
    await expect(
      setup().execute("cancelled", { paths: ["keep.txt"] }, controller.signal, undefined, { cwd }),
