...
const stringArray = (value: unknown, path: string): string[] | undefined => {
  if (value === undefined) {return undefined;}
  if (!Array.isArray(value)) {return fail(path, "must be an array of strings");}
  return value.map((entry, index) => {
    if (typeof entry !== "string") {return fail(`${path}.${index}`, "must be a string");}
    return entry;
  });
};
...
const stringMap = (value: unknown, path: string): Record<string, string> | undefined => {
  if (value === undefined) {return undefined;}
  if (!isObject(value)) {return fail(path, "must be an object whose values are strings");}
...
const aliasedValue = (
  ...
  aliases: { camel: string; snake: string },
  path: string,
): unknown => {
...
const parseOAuth = (value: unknown, path: string): OAuthConfig | undefined => {
  ...
  const clientId = optionalString(
    aliasedValue(value, { camel: "clientId", snake: "client_id" }, path),
    `${path}.clientId`,
  );
  const clientSecret = optionalString(
    aliasedValue(value, { camel: "clientSecret", snake: "client_secret" }, path),
    `${path}.clientSecret`,
  );
  const scope = optionalString(value.scope, `${path}.scope`);
  const redirectUri = optionalString(
    aliasedValue(value, { camel: "redirectUri", snake: "redirect_uri" }, path),
    `${path}.redirectUri`,
  );
...
const parseServer = (name: string, value: unknown): McpServerConfig => {
  ...
  return hasCommand
    ? parseStdioServer(path, value, disabled)
    : parseHttpServer(path, value, disabled);
};
...
export const parseMcpConfig = (value: unknown): McpServerMap => {
  if (!isObject(value)) {return fail("mcpServers", "configuration root must be an object");}
  if (!isObject(value.mcpServers)) {return fail("mcpServers", "must be an object");}
...
export const loadGlobalMcpConfig = (): Promise<McpServerMap> =>
  loadMcpConfigFile(globalMcpConfigPath());
