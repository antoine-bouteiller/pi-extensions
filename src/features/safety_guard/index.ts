...
import { handleToolCall } from './guard.js'
  ...
  implementation: {
    register: (pi: ExtensionAPI, runtime: AppRuntime): void => {
      ...
      pi.registerTool({
        ...
        execute: makeToolExecutor(runtime)(({ ctx, params, signal }) => run(params, signal, ctx), { interruptOnAbort: false }),
        label: 'Safe Remove',
        name: 'safe_rm',
