...
| --------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Non-goal  | `[KD-3]`   | Inline tool bridges omit the standard executor behavior.                                                                                                                | `src/features/sub_agents/agents.ts:442,589,656,703,741,796,862`. Every non-`sub_agents` site is migrated: `makeToolExecutor` now passes a `ToolInvocation` record and takes `interruptOnAbort` for cooperative bodies.                                                                                                   |
| Non-goal  | `[KD-5]`   | Inline command bridges.                                                                                                                                                 | `src/features/sub_agents/agents.ts:1104,1124`. `prompt_rewind` and `mcp` now use `makeCommandHandler`.                                                                                                                                                                                                                   |
...
| Non-goal  | `[KD-14a]` | Direct runtime adapters outside approved owners.                                                                                                                        | `src/features/mcp/oauth.ts:353` (the MCP SDK awaits `OAuthClientProvider`); `src/features/sub_agents/agents.ts:302,1038`; `src/features/sub_agents/peek.ts:439`. `comment_checker` and `caffeinate` are migrated; `status_panel/panel.ts:54,56,58` is reclassified as permanent service extraction.                      |
