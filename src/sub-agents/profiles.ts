...
export const AGENT_PROFILE_NAMES = Object.freeze([
  ...
  "implementer",
  "reviewer",
] as const satisfies readonly AgentProfileName[]);
...
const isGoogleCandidate = (model: AvailableModel): boolean =>
  GOOGLE_PROVIDER_PATTERN.test(model.provider) || GOOGLE_MODEL_PATTERN.test(model.id);
...
export const getAgentProfileNames = (
  registry?: Readonly<Record<string, AgentConfig>>,
): readonly string[] => (registry ? Object.freeze(Object.keys(registry)) : AGENT_PROFILE_NAMES);

export const getAgentProfilesDescription = (
  registry?: Readonly<Record<string, AgentConfig>>,
): string => {
...
export const resolveAgentConfig = (
  ...
  const availableModels = Object.freeze(
    context.availableModels.map((model) => Object.freeze({ id: model.id, provider: model.provider })),
  );
  ...
  const allowedTools = Object.freeze(
    [...new Set(config.allowedTools.map((tool) => tool.trim()).filter(Boolean))],
  );
...
export const persistedProfileColor = (profile: unknown, color: unknown): ThemeColor =>
  typeof profile === "string" && THEME_COLORS.has(color as ThemeColor)
    ? (color as ThemeColor)
    : "muted";
