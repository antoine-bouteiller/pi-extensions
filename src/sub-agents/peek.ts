...
const stripPromptMarkers = (lines: string[]): string[] =>
  lines.map((line) => line.replace(OSC133_PROMPT_MARKER_RE, ""));
...
export class SubagentPeekOverlay {
  ...
  constructor(options: SubagentPeekOverlayOptions) {
    ...
    this.loadSession();
    this.rebuildChat();
    if (isPeekActive(options.info.id)) {this.connectSocket();}
  ...
  private handleSyncEvent(event: SyncEvent): void {
    ...
    for (const activeTool of event.activeTools ?? []) {this.applyActiveTool(activeTool);}
  }
  ...
  handleInput(data: string): void {
    ...
    if (this.handleScrollInput(data)) {this.tui.requestRender();}
  }
  ...
  render(width: number): string[] {
    ...
    const lines = [
      this.theme.fg("border", "╭") + this.renderHeader(innerWidth) + this.theme.fg("border", "╮"),
    ];
