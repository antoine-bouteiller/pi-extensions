...
const defaultSaveFullOutput = async (content: string): Promise<string> => {
...
const fetchResult = async ({
  ...
  onUpdate,
  params,
}: FetchResultOptions): Promise<AgentToolResult<WebfetchDetails>> => {
  ...
  try {
    ...
    if (timedOut) {throw new Error(`webfetch timed out after ${timeoutSeconds}s`, { cause: error });}
    if (externalSignal?.aborted) {throw new Error("webfetch was cancelled", { cause: error });}
    throw error;
  } finally {
