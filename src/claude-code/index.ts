...
export const parseCommandFrontmatter = (content: string): CommandFrontmatter => {
  ...
  const descriptionMatch = match?.groups?.frontmatter.match(
    /^\s*description\s*:\s*(?<description>.*?)\s*$/m,
  );
  const firstContentLine = body
    ...
    ?.replace(/^#+\s*/, "");
  const description =
    unquote(descriptionMatch?.groups?.description ?? "") ||
    firstContentLine ||
    "Claude Code command";
  return { body, description: description.slice(0, 1024) };
};

const commandLogicalName = (relativePath: string): string =>
  relativePath.slice(0, -extname(relativePath).length);
