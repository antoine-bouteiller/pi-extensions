...
describe('extension entrypoints', () => {
  ...
  test('registers the first-party tools and lifecycle handlers', () => {
    ...
    for (const entrypoint of [askUser, backgroundPoll, claudeCode, commentChecker, hashline, mcp, rules, safeRm, safetyGuard, statusPanel]) {
      entrypoint(fixture.pi)
    }

    expect([...fixture.state.tools.keys()].toSorted()).toEqual(['ask_user', 'background_poll', 'hashline_read', 'hashline_write', 'mcp', 'safe_rm'])
