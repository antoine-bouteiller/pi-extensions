 ...
 */
const inspectDirectoryTree = async (directory: string, cwd: string, signal: AbortSignal | undefined): Promise<void> => {
...
const validateTarget = async ({ input, cwd, roots, recursive, signal }: ValidateTargetOptions): Promise<ValidatedTarget> => {
  throwIfCancelled(signal)
  ...
  if (directory) {
    await inspectDirectoryTree(absolute, cwd, signal)
  }
...
const revalidateTarget = async ({ target, roots, cwd, signal }: RevalidateTargetOptions): Promise<void> => {
  throwIfCancelled(signal)
  ...
  if (directory) {
    await inspectDirectoryTree(target.absolute, cwd, signal)
  }
}
  ...
  for (const [index, first] of targets.entries()) {
    for (const second of targets.slice(index + 1)) {
      if (first.absolute === second.absolute || isDescendant(first.absolute, second.absolute) || isDescendant(second.absolute, first.absolute)) {
        throw new Error(`Deletion targets must be distinct and non-overlapping: ${first.input}, ${second.input}`)
      }
    }
  }
}
...
export default function safeRm(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      const targets = await Promise.all(params.paths.map((path) => validateTarget({ cwd, input: path, recursive, roots, signal })))
