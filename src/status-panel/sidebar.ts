...
const quotaPercent = (percent: number) =>
...
const statusRows = (statuses: readonly string[], theme: SidebarTheme) =>
  statuses.map((status) => paint(theme, "muted", sanitize(status))).filter((status) => sanitize(status));
