...
| `[KD-3.1]` Announcements    | A settlement notice becomes flushable only after the fixed `SETTLEMENT_GRACE_MS` (15 s) window, rather than immediately. Any wait, targeted or untargeted, consumes a queued settlement notice for its targets instead of letting it publish, so a conclusion is never both returned by a wait and announced while its notice is still queued. A wait remains repeatable after publication (NG-002). Warning notices are unaffected and remain immediate, subject to head-of-line FIFO order behind an unready settlement. | The grace window lets waits claim their conclusions before automatic publication, while preserving one delivery path per conclusion and immediate warnings within FIFO ordering. |
...
`empty_targets`, `duplicate_target`, or `unknown_agent`. Explicit targeted waits claim queued
settlement notices and install delivery claims for eligible running turns; after publication they are
repeatable reads, and they never conflict with another delivery owner or targeted waiter.
...
| 2026-09-06 | Define settlement-notice grace, wait consumption, and immediate warning ordering.                                                   | 3                 | Match the shipped announcement behavior and prevent duplicate conclusion delivery.                                |
