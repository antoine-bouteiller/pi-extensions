...
| `[KD-9.1]` Resolution/config split | Resolution produces a redacted persisted profile, including its effective turn deadline; a separate private worker config carries immutable setup but no credential value. The worker reconstructs and verifies the selected setup, but never selects alternatives                                                          | Records remain explainable without secrets, preserve the bound that applied to each turn, and ordinary Pi auth resolution through inherited environment plus configured `agentDir` avoids a custom credential channel |
...
allow-list, a context ceiling, a turn deadline, and optional description, thinking level, and color. The set of
profile keys is closed; a delegation naming anything else is refused during admission.
...
effective context ceiling, and effective turn deadline.
...
The effective deadline is resolved once with the profile as `profile.turnDeadlineMillis ?? 30 * 60 * 1000` and persisted with that turn, so a resumed worker uses the deadline selected for its own spawn.
...
| Persisted/profile config split    | Assert `PersistedResolvedProfile` contains only key, provider/model, prompt, exact tools, thinking, ceiling, and resolved turn deadline. Assert closed JSON `WorkerConfig` also contains cwd, agentDir, project trust, and fixed resource/memory policies, contains no credential value, is never persisted/logged, and rejects unknown fields. The worker uses inherited environment plus configured agentDir for auth, loads configured extensions normally, exposes only exact resolved tool IDs, and fails mismatches pre-ready without fallback.                                            |
