...
const requireChildProcess = <ChildProcess,>(childProcess: ChildProcess | undefined): ChildProcess => {
  if (!childProcess) {throw new Error("expected the agent to own a child process");}
  return childProcess;
};
...
describe("child process lifecycle", () => {
  ...
  processTest("hibernates after settle and lazily restarts the persisted session", async () => {
    ...
    try {
      ...
      const secondPid = requireChildProcess(
        manager.getAgentInfo("worker", parentSessionId).childProcess,
      ).pid;
  ...
  processTest("reconciles owned children without risking PID-reuse kills", async () => {
    ...
    try {
      ...
      const orphanPid = requireChildProcess(
        owner.getAgentInfo("orphan", parentSessionId).childProcess,
      ).pid;
  ...
  processTest(
    ...
    async () => {
      ...
      const requireRenderer = (name: string): FakeRenderer => {
        ...
        if (!renderer) {throw new Error(`renderer ${name} was not registered`);}
        return renderer;
      };
      const requireWidget = (): FakeWidget => {
        if (!widget) {throw new Error("expected the sidebar widget to be registered");}
        return widget;
      };
      const pi = {
        getActiveTools() {
      ...
      try {
        ...
        expect(requireTool("spawn_agent").parameters.properties.agent_type.enum).toEqual([
          "scout",
          "librarian",
          "implementer",
          "reviewer",
        ...
        colorCalls.length = 0;
        requireTool("spawn_agent").renderCall(
          { agent_type: "librarian", task_name: "research" },
          theme,
        );
...
describe("subagent peek overlay", () => {
  const createOverlay = (columns = 80, rows = 20) => {
    ...
    const tui = {
      requestRender() { /* No-op stub; tests only assert on rendered output. */ },
    ...
    return new SubagentPeekOverlay({
      done: () => { /* No-op stub; navigation is not exercised in these tests. */ },
      info,
      theme,
      tui,
    });
