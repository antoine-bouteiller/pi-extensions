...
import { Context, Deferred, Effect, Exit, Fiber, Layer, Queue, Ref, Scope, FileSystem } from 'effect'
...
const harness = (root: string, options: HarnessOptions = {}) =>
  Effect.gen(function* () {
    ...
    const store: SubagentStoreApi = {
      ...
      createLog: (agentId: string, _turn: number) =>
        Effect.gen(function* () {
          ...
          return join(root, `${agentId}-${_turn}-${logCreates}.log`)
        }),
      createSession: (_agentId: string) =>
        options.createSessionFails === true
          ...
          : Effect.succeed({ runDirectory: root, sessionPath: join(root, 'session.json') }),
    ...
    yield* fs.writeFileString(join(root, 'session.json'), '{}')
    yield* fs.chmod(join(root, 'session.json'), 0o600)
...
describe('SubagentOrchestrator', () => {
  it.scoped('shares initialization and resolves cheap profile failures before a claim', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-' }).pipe(Effect.flatMap((path) => fs.realPath(path)))
  ...
  it.scoped('retries initialization after the owner fails', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-initialize-retry-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('rejects an oversized task frame before creating fresh artifacts', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-' }).pipe(Effect.flatMap(fs.realPath))
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.provide(
        Effect.gen(function* () {
          ...
          yield* good.emit(bytes({ agent_id: goodAgentId, command_id: 'initial', session_path: join(root, 'session.json'), turn: 1, type: 'ready' }))
          ...
          yield* bad.emit(bytes({ agent_id: 'agent-2', command_id: 'initial', session_path: join(root, 'session.json'), turn: 1, type: 'ready' }))
  ...
  it.scoped('rejects symlinked, non-regular, and escaping ready session paths', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-path-' }).pipe(Effect.flatMap((path) => fs.realPath(path)))
      const outside = yield* fs.makeTempDirectory({ prefix: 'orchestrator-outside-' })
      const regular = join(root, 'regular.json')
      const linked = join(root, 'linked.json')
      const directory = join(root, 'directory')
      const escaping = join(outside, 'escaping.json')
      yield* fs.writeFileString(regular, '{}')
      yield* fs.writeFileString(escaping, '{}')
      yield* fs.chmod(regular, 0o600)
      yield* fs.chmod(escaping, 0o600)
      yield* fs.symlink(regular, linked)
  ...
  it.scoped('settles stdout EOF and failures after readiness', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-stdout-' }).pipe(Effect.flatMap((path) => fs.realPath(path)))
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.provide(
        Effect.gen(function* () {
          ...
          yield* TestClock.adjust('1 millis')
          const [child] = fake.children
          yield* child.emit(bytes({ agent_id: 'agent-1', command_id: 'initial', session_path: join(root, 'session.json'), turn: 1, type: 'ready' }))
          yield* Fiber.join(spawned)
  ...
  it.scoped('retains pre-ready and cancelled workers until identity verification releases their leases', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-cleanup-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('keeps an unverifiable cleanup reservation across a replacement generation', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-retained-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('settles storage failures as agent_failed and routine failures as durable values', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-settlement-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('persists a failed result when writing the original terminal result fails', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-settlement-fallback-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('preserves a committed terminal result when lease cleanup reports failure', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-settlement-committed-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('does not retain a running agent when terminal persistence remains unavailable', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-settlement-unavailable-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('classifies oversized worker frames before and after readiness', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-oversized-frame-' }).pipe(Effect.flatMap(fs.realPath))
      ...
      yield* Effect.provide(
        Effect.gen(function* () {
          ...
          const readyFrame = bytes({
            agent_id: 'agent-3',
            command_id: 'initial',
            session_path: join(root, 'session.json'),
            turn: 1,
            type: 'ready',
          })
  ...
  it.scoped('drains a delayed result before classifying a fast worker exit', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-fast-exit-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('hands a still-live settled worker to cleanup while charging one capacity slot', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-settled-cleanup-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('hands off a child when session close races provisional registration', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-race-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('closes live generations when the service layer is disposed', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-dispose-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { closeExits: true, profiles: ['scout'] })
  ...
  it.scoped('gives session close five virtual seconds and releases a racing exit only once', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-close-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('deletes all mismatch artifacts and leases a worker before cleanup handoff', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-mismatch-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('deletes a resumed provisional mismatch and frees its predecessor task name', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-resumed-provisional-mismatch-' }).pipe(Effect.flatMap(fs.realPath))
      ...
      yield* Effect.provide(
        Effect.gen(function* () {
          ...
          yield* TestClock.adjust('1 millis')
          yield* ready(fake.children[2], 'agent-2', join(root, 'session.json'))
  ...
  it.scoped('deletes a resumed turn on identity mismatch without signalling it', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-resumed-mismatch-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { contextCeiling: 10_000, profiles: ['scout'], termination: ['mismatch'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
          ...
          yield* ready(fake.children[2], frameAgentId(fake.children[2].writes()[0]) ?? 'missing', join(root, 'session.json'))
  ...
  it.scoped('reconciles persisted runs before pruning and preserves unverifiable ownership', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-reconcile-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('does not terminate a child retained by a live owner and leaves legacy ownership fail-safe', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-owner-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('uses a real absolute worker entrypoint and classifies oversized artifacts', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-worker-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('reads durable settlements repeatedly, lists them, and preserves explicit wait order', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-reads-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('validates explicit wait targets atomically and rejects empty omitted snapshots', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-wait-validation-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('snapshots omitted waits and orders omitted wait_all results by task name', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-omitted-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('does not add work created after an omitted wait begins to its snapshot', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-snapshot-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('restores cancelled omitted waitOne claims', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-cancelled-wait-one-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('restores cancelled omitted waitAll claims', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-cancelled-wait-all-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('delivers an interrupt result only once when an omitted wait has claimed it', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-interrupt-wait-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('does not restore an abandoned omitted wait claim when an interrupt is cancelled', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-abandoned-interrupt-wait-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'], termination: ['stillAlive'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('restores an interrupted claim after interrupt cancellation during grace', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-cancelled-interrupt-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'], termination: ['stillAlive'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('does not duplicate an already-notified settlement when a settled interrupt is cancelled', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-cancelled-notified-interrupt-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('restores losing omitted waitOne claims for later notification', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-wait-one-claims-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('chooses the earliest settled explicit waitOne target', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-wait-one-order-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('atomically reserves the one running-turn follow-up', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-concurrent-steer-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('bounds notification batches and does not replay consumed notices after a session restart', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-notices-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { closeExits: true, profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.provide(
        Effect.gen(function* () {
          ...
          yield* TestClock.adjust('1 millis')
          yield* ready(fake.children[2], 'agent-3', join(root, 'session.json'))
          ...
          yield* TestClock.adjust('1 millis')
          yield* ready(fake.children[4], 'agent-4', join(root, 'session.json'), 2)
  ...
  it.scoped('refuses resume at the exact projected context ceiling', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-resume-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('refuses resume when the resolved model reserves more output than the fallback', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-reserve-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('admits resumes using token estimates and the resolved model output limit', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-estimate-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('re-resolves resumes and validates open-mode readiness and its timeout', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-open-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { contextCeiling: 10_000, profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
          ...
          yield* fs.writeFileString(alternate, '{}')
          yield* fs.chmod(alternate, 0o600)
          const mismatch = yield* Effect.forkChild(orchestrator.send('open', admission, 'source', 'next'))
          yield* TestClock.adjust('1 millis')
          const config = fake.children[1].writes()[0] ?? ''
          expect(config).toContain(`"canonical_path":"${join(root, 'session.json')}"`)
          ...
          yield* TestClock.adjust('1 millis')
          yield* ready(fake.children[2], 'agent-1', join(root, 'session.json'), 2)
          ...
          yield* TestClock.adjust('1 millis')
          yield* ready(fake.children[3], 'agent-2', join(root, 'session.json'))
  ...
  it.scoped('panic interrupts a launch reserved after its snapshot would otherwise escape', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-panic-starting-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('reports not_ready while a completed agent resume is provisional', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-resume-starting-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { contextCeiling: 10_000, profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('interrupts durably, retains live capacity, and preserves unrelated panic notices', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-interrupt-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('does not let a mismatched deadline settle a same-name replacement', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-mismatched-deadline-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('ignores late old-generation frames and exit after replacement', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-late-generation-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'], termination: ['stillAlive'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('refuses failed and interrupted source turns independently before resume admission', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-not-resumable-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('refuses a captured resume after a same-name replacement commits', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-resume-replacement-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('returns unknown_agent for a stale-generation resume target after replacement', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-stale-resume-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { closeExits: true, profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('makes a cancelled foreground result notification-eligible', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-cancelled-foreground-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
  ...
  it.scoped('hands an explicitly cancelled admitted caller to cleanup without releasing its live slot twice', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-cancelled-' }).pipe(Effect.flatMap(fs.realPath))
      const fake = yield* harness(root, { profiles: ['scout'], termination: ['stillAlive'] })
      yield* Effect.provide(
        Effect.gen(function* () {
          const orchestrator = yield* SubagentOrchestrator
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.provide(
        Effect.gen(function* () {
          ...
          yield* ready(fake.children[0], agentId ?? 'missing', join(root, 'session.json'))
  ...
  it.scoped('fails closed when session usage is unavailable and admits conservatively measured usage', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-context-' }).pipe(Effect.flatMap(fs.realPath))
      ...
      yield* Effect.provide(
        Effect.gen(function* () {
          ...
          yield* TestClock.adjust('1 millis')
          yield* ready(unavailable.children[0], 'agent-1', join(root, 'session.json'))
      ...
      yield* Effect.provide(
        Effect.gen(function* () {
          ...
          yield* TestClock.adjust('1 millis')
          yield* ready(measured.children[1], 'agent-1', join(root, 'session.json'), 2)
          yield* Effect.yieldNow
  ...
  it.scoped('releases failed resume startup capacity and restores its follow-up allowance', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-resume-release-' }).pipe(Effect.flatMap(fs.realPath))
  ...
  it.scoped('has one durable winner when results race inactivity and deadline supervision', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectory({ prefix: 'orchestrator-winners-' }).pipe(Effect.flatMap(fs.realPath))
