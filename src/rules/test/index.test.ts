...
describe("rule parsing", () => {
  ...
  test("keeps plain rules and diagnoses malformed frontmatter", () => {
    ...
    expect(parseRuleFrontmatter("---\npaths: src/**").diagnostic).toContain(
...
describe("rule loading", () => {
  test("loads always-on .claude and .agents rules globally and locally", async () => {
    ...
    const untrusted = await fixture.invoke<PromptResult>(
      "before_agent_start",
      { systemPrompt: "Base" },
      false,
    );
    ...
    expect(trustedPrompt.indexOf("Global Claude")).toBeLessThan(
      trustedPrompt.indexOf("Project Claude"),
    );
  });
  ...
  test("truncates oversized rules with a pointer to the source", async () => {
    const fixture = await createFixture();
    await writeFixture(
      join(fixture.projectDirectory, ".agents/rules/large.md"),
      "x".repeat(13_000),
    );
    ...
    expect(result?.systemPrompt).toContain(
      "[Rule truncated. Read full rule: .agents/rules/large.md]",
    );
...
describe("path-scoped injection", () => {
  test("injects matching scoped rules after file tools and deduplicates by target and content", async () => {
    ...
    expect(prompt?.systemPrompt).toContain(
      "- .agents/rules/typescript.md — applies to: src/**/*.ts, !src/generated/**",
    );
    ...
    expect(
      await fixture.invoke<ToolResult>("tool_result", readEvent("src/lib/main.ts")),
    ).toBeUndefined();
    await fixture.invoke("session_tree", {});
    const afterTreeNavigation = await fixture.invoke<ToolResult>(
      "tool_result",
      readEvent("src/lib/main.ts"),
    );
    expect(afterTreeNavigation?.content.at(-1)?.text).toContain("Use strict TypeScript.");
    expect(
      await fixture.invoke<ToolResult>("tool_result", readEvent("src/generated/types.ts")),
    ).toBeUndefined();
  ...
  test("keeps local scoped rules out of untrusted projects but applies global scoped rules", async () => {
    ...
    await Promise.all([
      writeFixture(
        join(fixture.homeDirectory, ".claude/rules/global.md"),
        "---\npaths: src/**\n---\nGlobal scope",
      ),
      writeFixture(
        join(fixture.projectDirectory, ".claude/rules/local.md"),
        "---\npaths: src/**\n---\nLocal scope",
      ),
    ]);
  ...
  test("extracts built-in and hashline file paths", () => {
    expect(extractToolPaths(readEvent("src/main.ts") as never, "/project")).toEqual([
      "/project/src/main.ts",
    ]);
    expect(
      extractToolPaths(
